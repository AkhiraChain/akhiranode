...
# IBC localnet tool: https://github.com/Sifchain/akhirachain-deploy/commit/0424d2ec81f6f739a3a2cbc77feba1a6f297430d
...
chains = {
    ...
    "akhirachain": {"binary": "akhiranoded", "relayer": "ibc"},
}
...
configs = {
    "local": {
        ...
        "akhirachain": "http://127.0.0.1:26657",
...
# Generates a akhiranoded key and stores it into test keyring. Returns the mnemonic that can be used to
# recreate it.
...
def run_tests_for_one_chain_in_one_direction(config, other_chain, direction_flag, number_of_iterations):
    # Assume we're always sending transactions between other_chain and akhirachain.
    # from_chain is chain sending the assets, to_chain is the receiving chain.
    from_chain = "akhirachain" if direction_flag else other_chain
    to_chain = other_chain if direction_flag else "akhirachain"
    ...
    fees = config["fees"]
    gas = config["gas"]
    akhirachain_proc = start_chain("akhirachain")
    ...
    mnemonic = add_new_key_to_keyring("akhirachain", from_account)

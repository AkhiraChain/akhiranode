...
akhiranoded_binary = "akhiranoded"
...
class EthereumToSifchainTransferRequest:
    ...
    akhiranoded_node: str = "tcp://localhost:26657"
...
def get_shell_output(command_line):
    ...
    if akhiranoded_binary in command_line and not "q auth account" in command_line:
        time.sleep(2)
...
def get_user_account(user, network_password):
    command_line = "yes " + network_password + f" | {akhiranoded_binary} keys show " + user + " -a"
...
def get_transaction_result(tx_hash, akhiranoded_node, chain_id):
    node = f"--node {akhiranoded_node}" if akhiranoded_node else ""
    chain_id_entry = f"--chain-id {chain_id}" if chain_id else ""
    command_line = f"{akhiranoded_binary} q tx {node} {tx_hash} {chain_id_entry} --output json"
...
def detect_errors_in_akhiranoded_output(result):
...
def send_from_akhirachain_to_akhirachain(
    ...
    time.sleep(4)
    # get_transaction_result(result["txhash"], transfer_request.akhiranoded_node, transfer_request.chain_id)
    return result

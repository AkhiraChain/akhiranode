...
def test_create_new_claim(claimType):
    akhirachain_address, akhirachain_name = create_new_sifaddr_and_key()
    keyring_backend = 'test'
    chain_id = 'localnet'
    from_address = 'sifnodeadmin'
    amount = '100000000010000000aku'
    send_sample_aku(from_address, akhirachain_address, amount, keyring_backend, chain_id, "")
    time.sleep(5)
    txnhash = (create_claim(akhirachain_address, claimType, keyring_backend, chain_id))
    ...
    try:
        ...
        assert str(expectedOutputTagsList[1]) == akhirachain_address

...
from test_utilities import get_required_env_var, SifchaincliCredentials, get_optional_env_var, ganache_owner_account, \
    get_shell_output_json, get_shell_output, detect_errors_in_akhiranoded_output, get_transaction_result, amount_in_wei, akhiranoded_binary
...
def get_pools(akhiranoded_node):
    node = f"--node {akhiranoded_node}" if akhiranoded_node else ""
    command_line = f"{akhiranoded_binary} q clp pools {node} --output json"
...
def swap_pool(
    ...
    txn = get_transaction_result(json_str["txhash"], transfer_request.akhiranoded_node, transfer_request.chain_id)
    tx = txn["tx"]

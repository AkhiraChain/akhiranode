...
from test_utilities import get_required_env_var, SifchaincliCredentials, get_optional_env_var, ganache_owner_account, \
    get_shell_output_json, get_shell_output, detect_errors_in_akiranoded_output, get_transaction_result, amount_in_wei, akiranoded_binary
...
def get_pools(akiranoded_node):
    node = f"--node {akiranoded_node}" if akiranoded_node else ""
    command_line = f"{akiranoded_binary} q clp pools {node} --output json"
...
def create_pool(
    ...
    cmd = " ".join([
        yes_entry,
        f"{akiranoded_binary} tx clp create-pool",
        ...
        node,
        sifchain_fees_entry,
        f"--home {credentials.akiranoded_homedir} ",
        "-y --output json"
    ])
...
def swap_pool(
    ...
    cmd = " ".join([
        yes_entry,
        f"{akiranoded_binary} tx clp swap",
        f"--from {transfer_request.sifchain_address}",
    ...
    txn = get_transaction_result(json_str["txhash"], transfer_request.akiranoded_node, transfer_request.chain_id)
    tx = txn["tx"]
...
def remove_pool_liquidity(
    ...
    cmd = " ".join([
        yes_entry,
        f"{akiranoded_binary} tx clp remove-liquidity",
        f"--from {transfer_request.sifchain_address}",
        f"--symbol {transfer_request.sifchain_symbol}",
        f"--wBasis {wBasis}",
        f"--asymmetry 0",
...
def add_pool_liquidity(
    ...
    cmd = " ".join([
        yes_entry,
        f"{akiranoded_binary} tx clp add-liquidity",
        f"--from {transfer_request.sifchain_address}",
        f"--symbol {transfer_request.sifchain_symbol}",
        f"--nativeAmount {transfer_request.amount}",
        f"--externalAmount {transfer_request.amount}",
...
def test_create_pools(
    ...
    # wait for balance
    test_utilities.wait_for_sifchain_addr_balance(sifaddress, "aku", target_aku_balance, basic_transfer_request.akiranoded_node)
    test_utilities.wait_for_sifchain_addr_balance(sifaddress, "ceth", target_ceth_balance, basic_transfer_request.akiranoded_node)

    pools = get_pools(basic_transfer_request.akiranoded_node)
    change_amount = 10 ** 18
    ...
    if pools is None:
        ...
        get_pools(basic_transfer_request.akiranoded_node)
        ...
        assert(test_utilities.get_sifchain_addr_balance(sifaddress, basic_transfer_request.akiranoded_node, "aku") == current_aku_balance)
        assert(test_utilities.get_sifchain_addr_balance(sifaddress, basic_transfer_request.akiranoded_node, "ceth") == current_ceth_balance)
...
def test_pools(
    ...
    # wait for balance
    test_utilities.wait_for_sifchain_addr_balance(sifaddress, "aku", target_aku_balance, basic_transfer_request.akiranoded_node)
    test_utilities.wait_for_sifchain_addr_balance(sifaddress, sifchain_symbol, target_new_currency_balance, basic_transfer_request.akiranoded_node)
    ...
    # wait for balance
    test_utilities.wait_for_sifchain_addr_balance(sifaddress2, "aku", target_aku_balance, basic_transfer_request.akiranoded_node)
    test_utilities.wait_for_sifchain_addr_balance(sifaddress2, sifchain_symbol, target_new_currency_balance, basic_transfer_request.akiranoded_node)
    ...
    current_aku_balance = current_aku_balance - change_amount - sifchain_fees_int
    assert(test_utilities.get_sifchain_addr_balance(sifaddress, basic_transfer_request.akiranoded_node, "aku") == current_aku_balance)
    assert(test_utilities.get_sifchain_addr_balance(sifaddress, basic_transfer_request.akiranoded_node, sifchain_symbol) == current_coin_balance)
    ...
    #assert(test_utilities.get_sifchain_addr_balance(sifaddress, basic_transfer_request.akiranoded_node, "aku") == current_aku_balance)
    #assert(test_utilities.get_sifchain_addr_balance(sifaddress, basic_transfer_request.akiranoded_node, sifchain_symbol) == current_ceth_balance)
    # no slippage if pool is perfectly balanced.

    # TODO: compute this precisely?
    slip_pct = 0.01
    balance = test_utilities.get_sifchain_addr_balance(sifaddress, basic_transfer_request.akiranoded_node, "aku")
    ...
    balance = test_utilities.get_sifchain_addr_balance(sifaddress, basic_transfer_request.akiranoded_node, sifchain_symbol)

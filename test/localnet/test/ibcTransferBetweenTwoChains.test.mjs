...
const chainAProps = getChainProps({ chain: "akhirachain", network: "testnet-1" });
...
test("ibc transfer between two chains", async () => {
  ...
  const channel =
    await $`ibc-setup channels --chain akhirachain --home ${chainBHome}/relayer/ | grep ${connection} | cut -d ' ' -f 1`;

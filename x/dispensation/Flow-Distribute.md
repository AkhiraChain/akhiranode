# USER FLOW 
## Context 
- Two users amara and zane want to do an airdrop to distribute aku to some recipients. The already have a list of recipients formatted as below .
```json
{
  "Output": [
    {
...
akiranoded keys add mkey --multisig amara,zane --multisig-threshold 2  
```
...
### Create offline tx
- Zane and amara needs to decide how much of the total funding they would provide individually . Looking at the output list above we need a total of 10aku . They decide to fund 5aku each .Based on this they will create an input file which looks like this.
```json
{
 "Input": [
  {
...
```shell
akiranoded tx dispensation create mkey airdrop-1 Airdrop input.json output.json --gas 200064128 --gas-prices 1.0aku --generate-only --from zane --keyring-backend file --node tcp://rpc-mainnet.sifchain.finance:80 --chain-id sifchain-mainnet >> offlinetx.json
```

### Zane Signs the transaction 
```shell
akiranoded tx sign --multisig sif1l7hypmqk2yc334vc6vmdwzp5sdefygj2ad93p5 --from zane offlinetx.json --keyring-backend file --node tcp://rpc-mainnet.sifchain.finance:80 --chain-id sifchain-mainnet >> sig-zane.json
```
...
```shell
akiranoded tx sign --multisig sif1syavy2npfyt9tcncdtsdzf7kny9lh777yqc2nd --from amara offlinetx.json --keyring-backend file --node tcp://rpc-mainnet.sifchain.finance:80 --chain-id sifchain-mainnet >> sig-amara.json
```
...
```shell
akiranoded tx multisign offlinetx.json mkey sig1.json sig2.json --keyring-backend file --node tcp://rpc-mainnet.sifchain.finance:80 --chain-id sifchain-mainnet >> signedtx.json
```

### Broadcast transaction to network
```shell
akiranoded tx broadcast signedtx.json --node tcp://rpc-mainnet.sifchain.finance:80 --chain-id sifchain-mainnet 
```
Sample output
```json
{

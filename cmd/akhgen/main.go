...
import (
	...
	"github.com/AkhiraChain/akhiranode/tools/akhgen"
	"github.com/spf13/cobra"
)

func main() {
	rootCmd := &cobra.Command{Use: "akhgen"}

	_networkCmd := networkCmd()
	_networkCmd.PersistentFlags().String("bond-amount", "1000000000000000000000000aku", "bond amount")
	_networkCmd.PersistentFlags().String("mint-amount", "999000000000000000000000000aku", "mint amount")
	...
	_nodeCreateCmd.PersistentFlags().String("bond-amount", "1000000000000000000000000aku", "bond amount")
	_nodeCreateCmd.PersistentFlags().String("mint-amount", "999000000000000000000000000aku", "mint amount")
...
func networkCreateCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			network := akhgen.NewAkhgen(&args[0]).NewNetwork(keyringBackend)
...
func networkResetCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			akhgen.NewAkhgen(&args[0]).NetworkReset(args[1])
		},
	}
}
...
func nodeCreateCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			node := akhgen.NewAkhgen(&args[0]).NewNode(keyringBackend)
			node.Moniker = args[1]
...
func nodeResetStateCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) == 1 {
				akhgen.NewAkhgen(&args[0]).NodeReset(nil)
			} else {
				akhgen.NewAkhgen(&args[0]).NodeReset(&args[1])
			}
		},
	}
}
...
func keyGenerateMnemonicCmd() *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.MaximumNArgs(0),
		Run: func(cmd *cobra.Command, args []string) {
			akhgen.NewAkhgen(nil).KeyGenerateMnemonic("", "")
		},
	}
}
...
func keyRecoverFromMnemonicCmd() *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.MinimumNArgs(1),
		Run: func(cmd *cobra.Command, args []string) {
			akhgen.NewAkhgen(nil).KeyRecoverFromMnemonic(args[0])
		},
	}
}
